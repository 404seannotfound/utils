...
func buildExplainPrompt(files []filePair, instruction string) string {
	var b strings.Builder
	b.WriteString("You are a senior software engineer. Read the provided file(s) and write a clear, concise explanation.\n")
	b.WriteString("Focus on purpose, key functions/types, data flow, noteworthy patterns, and potential pitfalls.\n")
	b.WriteString("If multiple files are provided, give a holistic overview and how parts fit together.\n\n")
...
func run() int {
	file := flag.String("file", "", "Single file to explain")
	dir := flag.String("dir", "", "Directory to snapshot and explain holistically")
	instructionArg := flag.String("instruction", "", "Optional instruction text, or file:<path> to read from file")
	model := flag.String("model", "gpt-oss:20b", "Ollama model name")
	exclude := flag.String("exclude", ".git,.hg,.svn,.venv,node_modules,*.png,*.jpg,*.jpeg,*.gif,*.webp,*.pdf,*.zip,*.mp4,*.mov,*.avi,*.mkv,*.mp3,*.wav,*.ogg,*.flac,*.dll,*.so,*.dylib,*.o,*.a,*.exe,*.class,*.jar", "Comma-separated patterns to exclude (dir mode)")
	maxChars := flag.Int("max-chars", 300000, "Max total characters of repo content to include (dir mode)")
	...
	// Default instruction depending on mode
	defaultInstrFile := "Provide a basic explanation of what this file does and anything significant."
	defaultInstrDir := "Provide a holistic explanation of this code snapshot: purpose, main components, how files relate, and any significant patterns or risks."

	instrVal := strings.TrimSpace(*instructionArg)
	if instrVal == "" {
		if *file != "" {
			instrVal = defaultInstrFile
		} else {
			instrVal = defaultInstrDir
		}
	}

	instr, err := loadInstruction(instrVal)
	if err != nil {
	...
	if *file != "" {
		...
		if !isProbablyText(abs, 2048) {
			fmt.Fprintln(os.Stderr, "File does not look like text; refusing to include.")
			return 3
		}
		...
		pairs = []filePair{{rel: filepath.Base(abs), text: string(b)}}
	} else {
	...
	prompt := buildExplainPrompt(pairs, instr)
